    ...
    <div id='task-create-modal' class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
...
function create()
 {
   ...
   var data=$("#task-create-form").serialize();
  ...
  $.ajax({
   type: 'POST',
    url: '<?php
 echo Yii::app()->createAbsoluteUrl("task/create"); ?>',
   data:data,
success:function(data){
                //alert("succes:"+data); 
                if(data!="false")
                 {
                  $('#task-create-modal').modal('hide');
                  renderView(data);
...
{
  $('#task-create-form').each (function(){
  this.reset();
   });
