...
class MeetingController extends CController
{
	...
	public function actionView()
	{
		...
		$this->widget('bootstrap.widgets.TbButton', array(
		 ...
		 echo "<div class='printableArea'>";
	         echo "<h1>View Meeting #".$model->id."</h1><hr />";
	...
	{	
		...
		$this->performAjaxValidation($model,"meeting-create-form");
            if(Yii::app()->request->isAjaxRequest)
	       {
	...
	public function actionUpdate()
	{
	    ...
	    $id=isset($_REQUEST["id"])?$_REQUEST["id"]:$_REQUEST["Meeting"]["id"];
	    $model=$this->loadModel($id);
			    
	    // Uncomment the following line if AJAX validation is needed
	      $this->performAjaxValidation($model,"meeting-update-form");
	...
	public function actionIndex()
	{
                ...
                if(isset($_GET['Meeting']))
		{
                       ...
                       if (!empty($model->activity_id)) $criteria->addCondition('activity_id = "'.$model->activity_id.'"');
                     
                    	
                       if (!empty($model->category_id)) $criteria->addCondition('category_id = "'.$model->category_id.'"');
                     
                    	
                       if (!empty($model->description)) $criteria->addCondition('description = "'.$model->description.'"');
                     
                    	
                       if (!empty($model->startdatetime)) $criteria->addCondition('startdatetime = "'.$model->startdatetime.'"');
                     
                    	
                       if (!empty($model->enddatetime)) $criteria->addCondition('enddatetime = "'.$model->enddatetime.'"');
                     
                    	
                       if (!empty($model->url)) $criteria->addCondition('url = "'.$model->url.'"');
                     
                    	
                       if (!empty($model->attendee)) $criteria->addCondition('attendee = "'.$model->attendee.'"');
                 ...
                 $session['Meeting_records']=Meeting::model()->findAll($criteria); 
        ...
        public function actionGenerateExcel()
	{
            ...
            $session->open();		
             ...
             if(isset($session['Meeting_records']))
               {
                $model=$session['Meeting_records'];
               }
               else
                 $model = Meeting::model()->findAll();
        ...
        public function actionGeneratePdf() 
	{
		...
		$pdf->SetTitle('Meeting Report');
		$pdf->SetSubject('Meeting Report');
