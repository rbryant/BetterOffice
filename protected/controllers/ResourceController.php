...
class ResourceController extends CController
{
	...
	public function actionView()
	{
		...
		$this->widget('bootstrap.widgets.TbButton', array(
		 ...
		 echo "<div class='printableArea'>";
	         echo "<h1>View Resource #".$model->id."</h1><hr />";
	...
	{	
		...
		$this->performAjaxValidation($model,"resource-create-form");
            if(Yii::app()->request->isAjaxRequest)
	       {
	...
	public function actionUpdate()
	{
	    ...
	    $id=isset($_REQUEST["id"])?$_REQUEST["id"]:$_REQUEST["Resource"]["id"];
	    $model=$this->loadModel($id);
			    
	    // Uncomment the following line if AJAX validation is needed
	      $this->performAjaxValidation($model,"resource-update-form");
	...
	public function actionIndex()
	{
                ...
                if(isset($_GET['Resource']))
		{
                       ...
                       if (!empty($model->payrate)) $criteria->addCondition('payrate = "'.$model->payrate.'"');
                     
                    	
                       if (!empty($model->userid)) $criteria->addCondition('userid = "'.$model->userid.'"');
                     
                    	
                       if (!empty($model->billrate)) $criteria->addCondition('billrate = "'.$model->billrate.'"');
                     
                    			
		}
                 $session['Resource_records']=Resource::model()->findAll($criteria); 
        ...
        public function actionGenerateExcel()
	{
            ...
            $session->open();		
             ...
             if(isset($session['Resource_records']))
               {
                $model=$session['Resource_records'];
               }
               else
                 $model = Resource::model()->findAll();
        ...
        public function actionGeneratePdf() 
	{
		...
		$pdf->SetTitle('Resource Report');
		$pdf->SetSubject('Resource Report');
