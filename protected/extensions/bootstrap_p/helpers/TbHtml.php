...
EOD;
    ...
    /**
     * Generates an Fontawsome Icon.
     * @param string $fa the font character.
     * @param array $htmlOptions additional HTML attributes.
     * @param string $tagName the fa HTML tag.
     * @return string the generated fa.
     */
    public static function fa($fa, $htmlOptions = array(), $tagName = 'i')
    {
        if (is_string($fa)) {
            if (strpos($fa, 'fa') === false) {
                $fa = 'fa ' . implode(' fa ', explode(' ', $fa));
            }
            self::addCssClass($fa, $htmlOptions);
            $color = TbArray::popValue('color', $htmlOptions);
            if (!empty($color) && $color === self::FA_COLOR_WHITE) {
                self::addCssClass('white', $htmlOptions);
            }
            return self::openTag($tagName, $htmlOptions) . parent::closeTag($tagName); // tag won't work in this case
        }
        return '';
